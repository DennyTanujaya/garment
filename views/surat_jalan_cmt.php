			...
			<div class="row">
				...
				<div class="col-75">
					<?php 
						...
						$query = "SELECT max(no_surat_jalan) as maxKode FROM surat_jalan_cmt";
						...
						$no_surat_jalan_dynamic = "SK/CMT/" . $dateSuratJalan . "/" . sprintf("%03s", $noUrut);
					?>
	...
	<div id="halaman">
		...
		<table border="1" class="demo-table">
			...
			if(isset($_POST['searchButton']) OR isset($_GET['search'])){
				...
				$result = mysqli_query($connect,"select * from surat_jalan_cmt WHERE id_karyawan LIKE '%$search%' OR no_surat_jalan LIKE '%$search%'");
				$query_mysql = mysqli_query($connect,"select * from surat_jalan_cmt WHERE id_karyawan LIKE '%$search%' OR no_surat_jalan LIKE '%$search%' LIMIT $mulai, $halaman")or die(mysql_error());
			} else {
				$result = mysqli_query($connect,"select * from surat_jalan_cmt");
				$query_mysql = mysqli_query($connect,"select * from surat_jalan_cmt LIMIT $mulai, $halaman")or die(mysql_error());
			}
			...
			<tr>
				...
				<td>
					<a href="details/detail_surat_jalan_cmt.php?id_karyawan=<?php echo $data['id_karyawan']; ?>" class="ahrefButton">Detail</a>	
					<?php if($data['status'] == 'Keluar'){ ?>
					<a href="../controllers/proses_update_surat_jalan_cmt.php?id=<?php echo $data['no_surat_jalan']; ?>" class="ahrefButton">Edit</a>
					<?php } ?>
				</td>
			</tr>
			<?php
