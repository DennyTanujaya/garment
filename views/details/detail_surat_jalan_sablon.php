<?php
	...
	if(!isset($_SESSION['nama_user'])){
		...
		if(isset($_GET['id_karyawan'])){
			$id = $_GET['id_karyawan'];
			$query_mysql = mysqli_query($connect,"select * from surat_jalan_sablon WHERE id_karyawan='$id'");
			$data = mysqli_fetch_array($query_mysql);
			$query_mysql_tanggal_penggajian = mysqli_query($connect,"select * from surat_jalan_sablon WHERE id_karyawan='$id' GROUP BY tanggal_gajian")or die(mysqli_error($connect));
		...
		<div class="container">
			...
			<table border="1" class="demo-table">
				...
				<tr>
					...
					<td><a href="../print/print_surat_jalan_sablon.php?id_karyawan=<?php echo $data['id_karyawan']; ?>&tanggal_penggajian=<?php echo $row[3]; ?>" class="ahrefButton" target='_BLANK'>Print</a>
