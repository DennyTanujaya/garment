			...
			<div class="row">
				...
				<div class="col-25">
					<?php 
						...
						$query = "SELECT max(no_surat_jalan) as maxKode FROM salary_cmt";
						...
						$no_surat_jalan_dynamic = "SK/BDR/" . $dateSuratJalan . "/" . sprintf("%03s", $noUrut);
					?>
	...
	<div id="halaman">
		...
		<table border="1" class="demo-table">
			...
			if(isset($_POST['searchButton']) OR isset($_GET['search'])){
				...
				$result = mysqli_query($connect,"select * from salary_cmt WHERE id_karyawan LIKE '%$search%' OR no_surat_jalan LIKE '%$search%'");
				$query_mysql = mysqli_query($connect,"select * from salary_cmt WHERE id_karyawan LIKE '%$search%' OR no_surat_jalan LIKE '%$search%' LIMIT $mulai, $halaman")or die(mysql_error());
			} else {
				$result = mysqli_query($connect,"select * from salary_cmt");
				$query_mysql = mysqli_query($connect,"select * from salary_cmt LIMIT $mulai, $halaman")or die(mysql_error());
			}
			...
			<tr>
				...
				<td>
					<a href="details/detail_salary_exp.php?id_karyawan=<?php echo $data['id_karyawan']; ?>" class="ahrefButton">Detail</a>
					<?php if($data['status'] == 'Keluar'){ ?>
					<a href="../controllers/proses_update_salary_exp.php?id=<?php echo $data['no_surat_jalan']; ?>" class="ahrefButton">Edit</a>
					<?php } ?>
