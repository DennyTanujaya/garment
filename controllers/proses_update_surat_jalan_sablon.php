<?php
	...
	if(!isset($_SESSION['nama_user'])){
		...
		if(isset($_GET['id'])){
			$id = $_GET['id'];
			$query_mysql = mysqli_query($connect,"select * from surat_jalan_sablon WHERE no_surat_jalan='$id'");
			$data = mysqli_fetch_array($query_mysql);
			$query_mysql_seri = mysqli_query($connect,"select * from surat_jalan_sablon WHERE no_surat_jalan='$id'");
			...
			if(isset($_POST['buttonSubmitUpdate'])){ //check if form was submitted
				...
				for($i = 0; $i < $jumlah; $i++){
					...
					//Insert Invoice
					mysqli_query($connect, "UPDATE surat_jalan_sablon SET quantity_masuk='$quantity[$i]', tanggal_masuk='$tanggal_masuk[$i]', status='Masuk' WHERE no_surat_jalan='$no_surat_jalan' AND no_seri='$no_seri[$i]'")or die(mysqli_error($connect));
					header("location:../views/surat_jalan_sablon.php?pesan='update'");
				}
			}
		} else {
...
<div class="content">
	...
	<div id="halaman">
		...
		<?php
			include "../config/connection.php";
			$id = $_GET['id'];
			$query_mysql = mysqli_query($connect,"select * from surat_jalan_sablon where no_surat_jalan = '$id'")or die(mysqli_error($connect));
			$data = mysqli_fetch_array($query_mysql);
		?>
		<div class="container">
			<h3>Update data</h3>
